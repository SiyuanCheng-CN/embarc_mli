            ...
            for (int dim1_idx = dim_start[1]; dim1_idx < dim_end[1]; ++dim1_idx) {
                for (int dim2_idx = dim_start[2]; dim2_idx < dim_end[2]; ++dim2_idx) {
                    for (int dim3_idx = dim_start[3]; dim3_idx < dim_end[3]; ++dim3_idx) {
                        ...
                        acc_T dst_acc_shf = mli_math_asr_rnd_fx(dst_acc, scale_shift);
                        acc_T dst_out_zp = mli_math_add_fx<acc_T>(dst_acc_shf, out_zp);
                        out_T dst_val = mli_math_cast_fx<acc_T, out_T>(dst_out_zp, 0);
                        dst_tensor_arr[dst_pos] = dst_val;
                    }
                }
            }
        }
